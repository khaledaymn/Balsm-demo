...
export class EmployeeVacationsComponent implements OnInit, OnDestroy {
        ...
        this.filterForm.valueChanges
          ...
          .subscribe(() => {
            if (this.filterForm.valid) {
              console.log("Vacation Form Value Changed:", this.filterForm.value)
  ...
  loadEmployeeVacations(): void {
    ...
    this.reportsService
      .getEmployeeVacations(this.employeeId()!, this.pageNumber(), this.pageSize(), year!, month!)
      .subscribe({
        next: (response) => {
          ...
          if (!response.employeeVacationDetails.length) {
            // this.showNotification("info", "لا توجد بيانات إجازات متاحة لهذه الفلاتر.")
          } else {
        ...
        error: (error) => {
          ...
          // this.showNotification("error", error.message || "حدث خطأ أثناء تحميل بيانات الإجازات.")
