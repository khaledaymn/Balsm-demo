import { Component, signal, type OnInit, type OnDestroy, computed } from "@angular/core"
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, EmployeeVacationsComponent, AbsenceReportComponent],
...
export class AttendanceReportsComponent implements OnInit, OnDestroy {
  ...
  filteredReports = computed(() => {
    ...
    return reports.filter(
      (report) => report.name.toLowerCase().includes(search) || report.email.toLowerCase().includes(search),
    )
  })
  ...
  employeeStats = computed(() => {
    ...
    return {
      ...
      totalWorkingHours: reports.reduce((sum, r) => sum + r.numberOfMonthlyWorkingHours, 0),
      totalLateHours: reports.reduce((sum, r) => sum + r.numberOfLateHours, 0),
      totalAbsentDays: reports.reduce((sum, r) => sum + r.numberOfAbsentDays, 0),
    }
  })
  ...
  private setupFormSubscriptions(): void {
    ...
    this.subscription.add(
      this.employeeFilterForm.valueChanges.pipe(debounceTime(300), distinctUntilChanged()).subscribe(() => {
  ...
  loadAttendanceReports(): void {
    ...
    this.reportsService.getAttendanceReport(this.pageNumber(), this.pageSize(), year!, month!).subscribe({
      next: (response) => {
  ...
  loadEmployeeAttendance(employeeId: string): void {
    ...
    const { reportType, dayDate, fromDate, toDate } = this.employeeFilterForm.value

    // Validate inputs
    if (reportType === 0 && (!dayDate || !this.employeeFilterForm.controls.dayDate.valid)) {
    ...
    this.reportsService.getEmployeeAttendanceAndLeaveReport(employeeId, reportType!, filterParams).subscribe({
      next: (response) => {
      ...
      error: (error) => {
        ...
        this.showNotification("error", error.message || "حدث خطأ أثناء تحميل تقرير الموظف.")
  ...
  getBadgeClass(value: number, threshold: number, type: "danger" | "success" = "danger"): string {
    if (type === "danger") {
